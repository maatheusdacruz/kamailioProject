...
class XMLParserX
{
    ...
    //
    // Fix for PHP 7+: two bugs fixed vs. original:
    //
    // 1. $i is now passed by reference so the caller's index advances past
    //    the subtree consumed by getchildren(). Without this, the outer
    //    getchildren() loop would re-process already-consumed elements,
    //    causing nested subtrees (TableJoins, ObjReferences, …) to be lost.
    //
    // 2. 'complete' elements (self-closing <Tag/> or <Tag></Tag>) never have
    //    children. The original code called getchildren() for them when the
    //    'value' key was absent (common for attribute-only elements), which
    //    caused unbounded recursion and exhausted all available RAM.
    function buildtag($thisvals, $vals, &$i, $type)
    {
        ...
        if ($type === 'complete') {
            ...
            // complete tags never have children – do NOT recurse
        } else {
            // open tag, recurse
