...
class BizDataObj_SQLHelper
{
    ...
    public function buildQuerySQL($dataObj)
    {
        ...
        // Fix: pass $dataObj->m_Database so BizDataSql uses the correct quote style
        // (backticks for MySQL, no quoting for PostgreSQL)
        $dataSqlObj->addMainTable($dataObj->m_MainTable, $dataObj->m_Database);
        // add join table
    ...
    public function buildUpdateSQL($dataObj)
    {
        ...
        {
            // Fix: pass $dataObj->m_Database for correct identifier quoting
            $queryString = QueryStringParam::formatQueryString(BizSystem::getDBTableQuoted($col, $dataObj->m_Database), "=", $val);
            if ($sql!="") $sql .= ", $queryString";
            else $sql .= $queryString;
        }

        // Fix: pass $dataObj->m_Database for correct table quoting
        $sql = "UPDATE " . BizSystem::getDBTableQuoted($dataObj->m_MainTable, $dataObj->m_Database) . " SET " . $sql;
    ...
    public function buildUpdateSQLwithCondition($dataObj, $setValue, $condition = null)
    {
        ...
        // Fix: pass $dataObj->m_Database for correct table quoting
        $sql = "UPDATE " . BizSystem::getDBTableQuoted($dataObj->m_MainTable, $dataObj->m_Database) ." SET ".$setValueStr;
    	if($condition)
        {
    ...
    {
        // Fix: pass $dataObj->m_Database for correct table quoting
        $sql = "DELETE FROM " . BizSystem::getDBTableQuoted($dataObj->m_MainTable, $dataObj->m_Database);
    ...
    public function buildInsertSQL($dataObj, $joinValues=null)
    {
        ...
        foreach($sqlFlds as $fldobj)
        {
            ...
            // Fix: pass $dataObj->m_Database for correct column quoting
            $sql_col .= BizSystem::getDBTableQuoted($col, $dataObj->m_Database) . ", ";
            //$sql_val .= $_val. ", ";
        ...
        if(is_array($joinValues))
        {
            foreach($joinValues as $joinColumn=>$joinValue)
            {
                ...
                // Fix: pass $dataObj->m_Database for correct column quoting
                $sql_col .= BizSystem::getDBTableQuoted($joinColumn, $dataObj->m_Database) . ", ";
                $sql_val .= "'".$joinValue. "', ";
            }
        }
        ...
        // Fix: pass $dataObj->m_Database for correct table quoting
        $sql = "INSERT INTO  " . BizSystem::getDBTableQuoted($dataObj->m_MainTable, $dataObj->m_Database) . " (" . $sql_col . ") VALUES (" . $sql_val.")";
        return $sql;
    }
