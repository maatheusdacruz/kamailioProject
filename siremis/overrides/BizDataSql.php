...
class BizDataSql
{
    ...
    // Fix: store the database name so all join/association quoting uses the correct driver
     ...
     * @param string $dbName    database name (from DataObj's m_Database)
     * @return void
     **/
    public function addMainTable($mainTable, $dbName=null)
    {
        $this->_mainTable = "$mainTable";
        $this->_dbName    = $dbName;
		$this->_tableJoins = " " . BizSystem::getDBTableQuoted($mainTable, $dbName) . " T0 ";
    }
    ...
    {
        // Fix: pass the stored db name so PostgreSQL tables are not backtick-quoted
		$table = BizSystem::getDBTableQuoted($tableJoin->m_Table, $this->_dbName);
    ...
    public function addAssociation($assoc)
    {
        ...
        elseif ($assoc["Relationship"] == "M-M")
        {
            ...
            // Fix: pass stored db name for correct quoting
            $xtable = BizSystem::getDBTableQuoted($assoc["XTable"], $this->_dbName);
